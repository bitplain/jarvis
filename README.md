...
Production API автоматически выполняет `alembic upgrade head` перед стартом `uvicorn`, поэтому таблица `runtime_settings` должна создаваться без ручного шага.
