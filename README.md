...
- `WHOOP_CLIENT_SECRET` — optional secret WHOOP OAuth client secret для Stage 4 WHOOP sync.
- `WHOOP_TOKEN_ENCRYPTION_KEY` — optional Fernet key для encrypted storage WHOOP access/refresh tokens.
...
- `GET /integrations/whoop/oauth/start` — one-time start для WHOOP OAuth, вызывается только из ссылки `/settings -> WHOOP`.
- `GET /integrations/whoop/oauth/callback` — WHOOP OAuth callback.
...
- `digest_policies` хранит две default policies: `personal_morning` и `work_start`;
- личный дайджест отправляется в `06:50 Europe/Moscow` и включает только scopes `personal`, `household`;
- рабочий дайджест отправляется в `09:00 Europe/Moscow` и включает только scope `work`;
- `system` events не включаются по умолчанию;
- `target_chat_id` не угадывается: его нужно задать через `/settings -> Дайджесты -> Использовать этот чат` из личного чата администратора;
...
`offline` обязателен для refresh token. WHOOP refresh token ротируется: после refresh сохраняются новый access token и новый refresh token. `score_state` для sleep/recovery/cycle может быть `SCORED`, `PENDING_SCORE` или `UNSCORABLE`; `PENDING_SCORE` и `UNSCORABLE` сохраняются как raw data и не считаются ошибкой sync.

Переменные окружения задаются вручную после merge через Railway UI или локальный `.env`; этот PR их не меняет:

- `WHOOP_ENABLED=false|true`
- `WHOOP_CLIENT_ID`
- `WHOOP_CLIENT_SECRET`
- `WHOOP_REDIRECT_URI`
- `WHOOP_TOKEN_ENCRYPTION_KEY`

Production redirect URL для Railway:

```text
https://jarvis-production-786d.up.railway.app/integrations/whoop/oauth/callback
```

`WHOOP_TOKEN_ENCRYPTION_KEY` — Fernet key. Локально его можно сгенерировать так, не коммитя значение:

```bash
uv run --python 3.12 --extra dev python -c "from app.services.secret_cipher import SecretCipher; print(SecretCipher.generate_key())"
```

Подключение:

1. Открыть `/settings -> WHOOP`.
2. Нажать `Подключить WHOOP`.
3. Перейти по одноразовой ссылке, действующей 10 минут.
4. Завершить OAuth в WHOOP.
5. Вернуться в Telegram и нажать `Синхронизировать сейчас`.
6. Проверить `/status`.

Данные хранятся в PostgreSQL:

- `whoop_integrations` — статус подключения, scopes, encrypted access/refresh tokens, expiry, profile JSON, sync diagnostics.
- `whoop_sleep_records` — raw sleep records с unique `(integration_id, whoop_sleep_id)`.
- `whoop_recovery_records` — raw recovery records с unique `(integration_id, cycle_id)`.
- `whoop_cycle_records` — raw cycle records с unique `(integration_id, cycle_id)`.
...
- access DB status.
- WHOOP enabled/configured, connected integrations, last sync и last error count без токенов/секретов.
