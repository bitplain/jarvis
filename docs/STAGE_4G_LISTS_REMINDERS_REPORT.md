...
Shopping add parser делит несколько позиций по запятой, точке с запятой, newline и простому connector `и`. Текущий bot mention внутри item payload вырезается перед сохранением: `@Home_ai_my_bot творожок` становится `творожок`.
...
PASS_SHOPPING_PARSER_SANITIZE_READINESS
```
