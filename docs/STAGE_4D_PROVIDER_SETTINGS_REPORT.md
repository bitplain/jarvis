...
Миграция таблицы выполняется автоматически: API service запускает `alembic upgrade head` перед стартом `uvicorn`. Stage 4E добавляет code-level startup migration guard, который защищает production даже если Railway UI Start Command переопределит `railway.api.toml`. Ручной запуск миграции не должен быть обязательным для работы кнопки.
...
- Повторный Refresh, повторный выбор уже активного provider и Telegram `message is not modified` не должны ломать webhook.
