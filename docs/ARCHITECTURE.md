...
- `app/services/helpdesk_vacation.py` — Stage 4L-3 режим отпуска, summary/review cursor, HTML formatting и кнопки manual review.
...
- `app/db/repositories/helpdesk_vacation.py` и `helpdesk_vacation_state` — один vacation state row по scope `default`.
- `app/bot/routers/helpdesk_tickets.py` — команды `/ticket`, `/helpdesk_vacation`, `/helpdesk_vacation_on`, `/helpdesk_vacation_off`, callbacks `hd_ticket:*` и `hd_vacation:*`.
...
Stage 4L-3 Vacation Mode:

- состояние хранится в `helpdesk_vacation_state`: `enabled`, `enabled_at`, `disabled_at`, `last_reviewed_at`, actor user ids и timestamps;
- при включённом отпуске `check_helpdesk_imap_mailbox` продолжает polling, baseline/high-water-mark и dedup, создаёт event/work item, но перед Telegram send фиксирует `notify_status=suppressed_vacation`, `error_code=vacation` и не вызывает notifier;
- suppression не считается failed notification и не блокирует продвижение `last_seen_uid`;
- `remind_helpdesk_tickets` при включённом отпуске не отправляет `waiting_ack`/`in_work` reminders и переносит active `next_reminder_at` на `now + reminder_interval_minutes`;
- при выключении отпуска активные reminders также переносятся на будущий нормальный интервал, поэтому backlog не рассылается задним числом;
- manual review берёт suppressed vacation events с `enabled_at` или после `last_reviewed_at`, группирует их по GLPI ticket id и обновляет `last_reviewed_at` только после успешной отправки review message;
- `/settings -> HelpDesk` и `/helpdesk_vacation*` используют те же access boundaries, что HelpDesk ticket controls.
...
`/status` читает HelpDesk IMAP diagnostics только из Redis/PostgreSQL: live IMAP connect из `/status` запрещён. Блок показывает enabled/configured, host configured/missing, port, ssl, masked username, folder, telegram chat id configured/missing, missing config keys, last check/success/error, pending/failed notification counters, vacation mode, vacation since, vacation new since start, vacation new since last review, vacation last reviewed и attention-предупреждение при failed notifications > 0.
