...
Если `STREAMING_GROUP_FALLBACK_ENABLED=true`, worker не использует `sendMessageDraft`, отправляет `sendChatAction typing`, provisional `Думаю...`, throttled `editMessageText` и финальный edit. Если edit failed, worker отправляет финальный `sendMessage`.
