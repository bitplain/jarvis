...
Для отвечаемого group message router сохраняет сообщение в regular memory группы и ставит arq job `process_llm_message` с `private=false`; worker отправляет финальный ответ в группу.
