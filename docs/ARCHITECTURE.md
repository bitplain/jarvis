...
`/status` читает HelpDesk IMAP diagnostics только из Redis/PostgreSQL: live IMAP connect из `/status` запрещён. Блок показывает enabled/configured, host configured/missing, port, ssl, masked username, folder, telegram chat id configured/missing, missing config keys, last check/success/error и event counters.
